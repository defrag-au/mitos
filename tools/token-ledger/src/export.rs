...
/// Emit a real token's series as Rust source, for widget development.
///
/// Everything is read through `projections`, the same path a frontend uses, so
/// a fixture cannot drift from what the real surface would be handed. Sampled
/// evenly across the domain rather than truncated — the tail of a token's life
/// is usually the flat part, and a fixture that stops early makes every chart
/// look livelier than the token was.
pub fn fixture(dir: &Path, token: &str, points: usize) -> Result<()> {
    use token_ledger_wire::sample;

    let spine = wire::decode_spine(&std::fs::read(dir.join(format!("{token}.spine.bin")))?)?;
    let detail = wire::decode_detail(&std::fs::read(dir.join(format!("{token}.detail.bin")))?).ok();
    let (lo, hi) = spine.domain;

    // The ONE sampling path — shared with runtime consumers (token-fluid
    // loads artifacts and calls the same function), so a fixture cannot
    // drift from what a live surface computes from the same files.
    let s = sample::sample(&spine, detail.as_ref(), points, 12);
    let n = s.rows.len();

    println!("// Generated by `token-ledger fixture --token {token}`. Do not hand-edit.");
    println!("//");
    println!("// {} cohorts, domain slots {lo}..{hi}.", s.cohorts.len());
    println!("// Supply {}, decimals {}.", s.supply, s.decimals);
    println!(
        "pub const COHORTS: [&str; {}] = [{}];",
        s.cohorts.len(),
        s.cohorts
            .iter()
            .map(|c| format!("{c:?}"))
    ...
    println!();
    println!("/// `(dex, key_basis, first_seen_unix)` — a pool does not exist before this.");
    println!(
        "pub const POOL_META: [(&str, &str, i64); {}] = [",
        s.pools.len()
    );
    for p in &s.pools {
        println!(
            "    ({:?}, {:?}, {}),",
            p.dex, p.key_basis, p.first_seen_unix
        );
    }
    println!("];");

    println!("/// Per-pool watched-asset reserve, parallel to `SERIES`. 0 = does not exist yet.");
    println!("pub const POOL_SERIES: [[i64; {}]; {n}] = [", s.pools.len());
    for row in &s.pool_series {
        println!("    {row:?},");
    }
    println!("];");
    ...
    if !s.top_holders.is_empty() {
        println!();
        println!("/// Party ordinals of the top wallet-cohort holders by peak balance.");
        println!("/// Addresses for these ordinals live in `parties.bin`.");
        println!(
            "pub const HOLDERS: [u32; {}] = [{}];",
            s.top_holders.len(),
            s.top_holders
                .iter()
                .map(|p| p.to_string())
        ...
        println!("/// Per-sample balance per top holder, plus a final RESIDUAL column —");
        println!("/// every other wallet-cohort holder summed. Parallel to `SERIES`.");
        println!(
            "pub const HOLDER_SERIES: [[i64; {}]; {n}] = [",
            s.top_holders.len() + 1
        );
        for row in &s.holder_series {
            println!("    {row:?},");
        }
        println!("];");
    }
...
pub fn inspect(dir: &Path, token: &str, at_slot: Option<u64>) -> Result<()> {
    ...
    match proj::cap_at(&spine, Some(&detail), slot) {
        Some(cap) => {
            ...
            // Per WHOLE token, via the one shared conversion — see
            // `AssetId::spot_ada`. Three surfaces each had their own `/ 1e6`
            // and each was wrong by a factor of a million until fixed.
            println!(
                "\nspot          {:.8} ADA/token",
                spine.asset.spot_ada(cap.spot_lovelace)
            );
