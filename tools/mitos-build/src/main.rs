...
struct Args {
    ...
    /// Build the wasm artifact with a faster, less-optimised
    /// profile — `opt-level = 1`, `lto = false`,
    /// `codegen-units = 16`. Default release artifacts use
...
async fn main() -> anyhow::Result<()> {
    ...
    if let Some(SubCmd::Prepare { module }) = args.cmd.as_ref() {
        ...
        let materialised = materialise_module(&single, args.fast)?;
...
///
/// `fast` swaps the workspace's `[profile.release]` block from
/// the prod settings (LTO + `codegen-units = 1` + `opt-level =
/// "z"`) to a CI-friendly profile (no LTO, 16 codegen units,
/// `opt-level = 1`). Wasm output is functionally identical —
/// emission content depends on source, not optimisation level
/// — but per-module wall time drops ~3-5x in our measurements.
fn materialise_module(spec: &SingleFileSpec, fast: bool) -> anyhow::Result<Materialised> {
    ...
    //
    // Two release-profile flavours:
    //   - default — production posture; LTO + 1 codegen unit +
    //     `opt-level = "z"`. Smallest, fastest module at run
    //     time; slow to build.
    //   - `--fast` — CI/iteration posture; no LTO, 16 codegen
    //     units, `opt-level = 1`. ~3-5x faster build, same
    //     emission content (goldens stay green).
    let profile_release = if fast {
        r#"[profile.release]
opt-level = 1
lto = false
codegen-units = 16
