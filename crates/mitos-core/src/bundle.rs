...
impl Bundle {
    /// Construct a bundle.
    ///
    /// Pre-retirement, `data_dir` held mitos's own state (the
    /// `subscriptions.redb` registry for the outbound `Replicator`).
    /// Platform-v2 wasm-module state lives under its own
    /// `--modules-dir`. The bundle currently has no shared
    /// non-module state, but the binary still accepts
    /// `--data-dir` so operator scripts don't need updating.
    pub fn new(
        domain: DomainAdapter,
        config: RootConfig,
        listen: SocketAddr,
    ...
    pub async fn run(self, exit: CancellationToken) -> anyhow::Result<()> {
        ...
        // outbound `Replicator` subscription model + its
        // `/_admin/subscriptions` admin surface retired alongside
        // the three legacy in-tree indexers — platform-v2 wasm
        // modules use the companion runtime's HTTPS subscribe path
        // instead (`/api/companions/subscribe`).
        ...
        // check can hit it without credentials. No sensitive data
        // exposed. Pre-retirement this also surfaced the
        // `Replicator` summary (per-status subscription counts);
        // platform-v2 module subscription state is queryable via
        // `/_admin/modules` instead.
...
/// (env vars, paths) — crucially, this does NOT acquire the WAL
/// lock, so it's safe to run while another mitos instance is live
...
// `/_admin/subscriptions` admin surface retired alongside the three
// legacy in-tree indexers. Platform-v2 wasm modules are managed via
// `/_admin/modules/*` (see `mitos_platform::admin`), and their
// companion subscriptions live under
// `/var/lib/mitos/modules/<id>/companions/`.
